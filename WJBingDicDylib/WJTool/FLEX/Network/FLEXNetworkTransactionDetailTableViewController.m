...
//    for(FLEXNetworkDetailSection* section in sections) {
//        [log appendFormat:@"%@->\n", section.title];
//        for (FLEXNetworkDetailRow* row in section.rows) {
//            if ([row.title isEqual:@"Request Body"]) {
...
//                    }
//                    [log appendFormat:@"\t%@->%@", row.title, [WJToolUtil data2HexStr:bodyData]];
//                }
//            } else if ([row.title isEqual:@"Response Body"]){
//                NSData *responseData = [[FLEXNetworkRecorder defaultRecorder] cachedResponseBodyForTransaction:self.transaction];
//                if ([responseData length] > 0) {
//                    [log appendFormat:@"\t%@->%@", row.title, [WJToolUtil data2HexStr:responseData]];
//                }
//            } else
//                [log appendFormat:@"\t%@->%@\n", row.title, row.detailText];
//        }
//    }
//    printf("%s\n", [[NSString stringWithFormat:@"%@", log] UTF8String]);
